   ...
   document.addEventListener('click', (e) => {
    ...
    bookContainer.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.card__status')) {
            ...
            const bookId = statusButton.closest('.card').id;
            const foundBookIndex = myLibrary.findIndex(book => book.id === bookId);
            const stringified = JSON.stringify(myLibrary);

            // Guard: if book not found, do nothing
            if (foundBookIndex === -1) return;

            const current = myLibrary[foundBookIndex].isRead;
            if (current === 'Unread') {
            ...
            localStorage.setItem('myLibrary', JSON.stringify(myLibrary));w
            statusButton.innerHTML = `Status: ${myLibrary[foundBookIndex].isRead} <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-refresh-cw-icon lucide-refresh-cw"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>`;
        }
